...
const RangeField: React.FC<{label: string; value: number; min: number; max: number; step?: number; onChange: (value: number) => void}> = ({label, value, min, max, step = 0.01, onChange}) => (
  <label className="inspector-range"><span>{label}<b>{Number(value).toFixed(step < 1 ? 2 : 0)}</b></span><input type="range" value={value} min={min} max={max} step={step} onChange={(event) => onChange(Number(event.target.value))} /></label>
);
...
const ItemInspector: React.FC<{plan: EditPlan; item: SceneItem; frame: number; onPlan: InspectorProps['onPlan']}> = ({plan, item, frame, onPlan}) => {
  ...
  return <div className="inspector-stack">
    <div className="inspector-heading"><div><small>{item.type}</small><h3>{item.name || item.type}</h3></div><label className="mini-check"><input type="checkbox" checked={item.enabled} onChange={(event) => patch({enabled: event.target.checked})} />On</label></div>
    ...
    <RangeField label="Scale X" value={item.transform.scale_x} min={0.1} max={3} onChange={(value) => patchTransform({scale_x: value}, false)} />
    <RangeField label="Scale Y" value={item.transform.scale_y} min={0.1} max={3} onChange={(value) => patchTransform({scale_y: value}, false)} />
    <RangeField label="Rotation" value={item.transform.rotation} min={-180} max={180} step={1} onChange={(value) => patchTransform({rotation: value}, false)} />
    <RangeField label="Opacity" value={item.transform.opacity} min={0} max={1} onChange={(value) => patchTransform({opacity: value}, false)} />
    <RangeField label="Corner radius" value={item.transform.border_radius} min={0} max={160} step={1} onChange={(value) => patchTransform({border_radius: value}, false)} />
    <details className="inspector-details">
      <summary>Crop</summary>
      {(['left','right','top','bottom'] as const).map((side) => <RangeField key={side} label={side[0].toUpperCase() + side.slice(1)} value={item.transform.crop?.[side] || 0} min={0} max={0.8} onChange={(value) => patchTransform({crop: {...item.transform.crop, [side]: value}}, false)} />)}
    </details>
    {(item.type === 'video' || item.type === 'audio' || item.type === 'sfx') && <details className="inspector-details" open>
      <summary>Audio</summary>
      <RangeField label="Volume" value={Number(item.volume ?? 1)} min={0} max={2} onChange={(value) => patch({volume: value}, false)} />
    ...
    {item.type === 'text' && <details className="inspector-details" open>
      ...
      <label className="inspector-field"><span>Content</span><textarea value={String(item.text || '')} onChange={(event) => patch({text: event.target.value})} /></label>
      <div className="inspector-grid two-col">
      ...
      <label className="inspector-field"><span>Font family</span><input value={String(style.fontFamily || '')} onChange={(event) => patch({style: {...style, fontFamily: event.target.value}})} /></label>
    ...
    <label className="mini-check"><input type="checkbox" checked={item.locked} onChange={(event) => patch({locked: event.target.checked})} />Lock layer</label>
  </div>;
};

const CaptionInspector: React.FC<{plan: EditPlan; item: SceneItem; frame: number; onPlan: InspectorProps['onPlan']}> = ({plan, item, frame, onPlan}) => {
...
export const Inspector: React.FC<InspectorProps> = ({plan, clipId, selectedIds, frame, onPlan}) => {
  ...
  return <aside className="inspector"><ItemInspector plan={plan} item={item} frame={frame} onPlan={onPlan} />{item.type === 'captions' && <CaptionInspector plan={plan} item={item} frame={frame} onPlan={onPlan} />}</aside>;
};
